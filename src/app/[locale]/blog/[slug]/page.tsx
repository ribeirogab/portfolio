...
export default async function Blog({
  ...
  const jsonLdContent = JSON.stringify({
    ...
    image: post.image
      ? `${DATA.url}${post.image}`
      : `${DATA.url}/${locale}/blog/${slug}/opengraph-image`,
    url: `${DATA.url}/${locale}/blog/${slug}`,
    author: {
      "@type": "Person",
      name: DATA.name,
    ...
    <section id="blog">
      ...
      <div className="flex justify-start gap-4 items-center">
        <Link
          ...
          aria-label={dict.ui.blog.backToBlog}
        >
          <ChevronLeft className="size-3 group-hover:-translate-x-px transition-transform" />
          {dict.ui.blog.backToBlog}
        </Link>
      </div>
      ...
      <nav className="mt-12 pt-8 max-w-2xl">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          {previousPost ? (
            <Link
              href={`/${locale}/blog/${getSlug(previousPost)}`}
          ...
          {nextPost ? (
            <Link
              href={`/${locale}/blog/${getSlug(nextPost)}`}
